...
public:
  ...
  */
  ...
  {
    // Mark phase - mark all roots and their successors
    ...
    // Sweep phase - deallocate unmarked objects and reset marks
    Collectable *sweep = root;
    Collectable *prev = nullptr;
    while (sweep != nullptr)
    {
      if (sweep->mark)
      {
        // Object is marked (reachable), keep it and reset mark for next GC
        sweep->mark = false;
        prev = sweep;
        sweep = sweep->next;
      }
      ...
      {
        // Object is not marked (unreachable), delete it
        Collectable *temp = sweep;
        sweep = sweep->next;
        if (prev != nullptr)
        {
          prev->next = sweep;
